				...
				if($params->get("point", "0") == "1" && !($order_id_before > 0)){
					...
					try{
						...
						if($order->customer_id > 0){
							...
							$pointLog->addCustomerRuledPoint($order->customer_id, $rule, Member_PointRuleModel::RULE_ORDER_SALES, ($order->subtotal > 0)?$order->subtotal:$order->total, null, $order->shop_comment."(".number_format(($order->subtotal > 0)?$order->subtotal:$order->total)."円)");
						}
