 ...
 * サイトデータのリストを取得する。
 */
class Facebook_Report_MessageListExcel extends Clay_Plugin_Module_List{
	function execute($params){
		...
		if($params->check("key")){
			...
			foreach($_SERVER["ATTRIBUTES"][$params->get("key")] as $user){
				...
				$excel->getActiveSheet()->getStyle($this->getColKey($startCol).$startRow.":".$this->getColKey($startCol).($startRow + 4 + count($user["messages"])))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFCCCCFF');
				$excel->getActiveSheet()->getStyle($this->getColKey($startCol).$startRow.":".$this->getColKey($startCol).($startRow + 4 + count($user["messages"])))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
				$excel->getActiveSheet()->getStyle($this->getColKey($startCol + $line).$startRow.":".$this->getColKey($startCol + $line).($startRow + 4 + count($user["messages"])))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
				...
				$index = 5;
				foreach($user["messages"] as $date => $mess){
			...
			$excel->getActiveSheet()->setTitle("メッセージ");

			ob_end_clean();
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="message_report.xls"');
